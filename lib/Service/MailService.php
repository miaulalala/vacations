...
class MailService {
	...
	public function __construct(IMailer $mailer,
		...
		IFactory $lFactory,
		IManager $notificationManager, IConfig $config) {
	 ...
	 */
	public function sendRequestEmail(Vacation $vacation): void {
		$user = $this->userManager->get($vacation->getUserId());
		if ($user === null) {
			throw new ServiceException('Could not find vacation requester');
		}
		$managers = $user->getManagerUids();
		if(!empty($managers)) {
			$toManagers = array_map(function (int $managerId) {
				$m = $this->userManager->get($managerId);
				return [$m->getEMailAddress() => $m->getDisplayName()];
			}, $managers);
		}

		if(empty($toManagers)) {
			$toManagers = [$this->config->getAppValue(Application::APP_ID, 'vacation_email')];
		}

		$fromEmail = Util::getDefaultEmailAddress('vacations-noreply');
		$fromName = $user->getDisplayName() . ' via Nextcloud Vacation app';

		$message = $this->mailer->createMessage()
			->setFrom([$fromEmail => $fromName])
			->setTo(...$toManagers);

		$template = $this->mailer->createEMailTemplate('vacation.book');
		$template->addHeader();

		//Subject
		$subject = $this->l10n->t('"%s" has sent a vacation request', [$user->getDisplayName()]);
		$template->setSubject($subject);

		// Heading
		$summary = $this->l10n->t("A vacation request by %s needs processing.", [$user->getDisplayName()]);
		$template->addHeading($summary);

		$bookingUrl = $this->urlGenerator->linkToRouteAbsolute('vacation.confirmBooking', ['token' => $vacation->getToken()]);
